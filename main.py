...
def publish_message(publisher, topic_path, data):
  ...
  data_string = json.dumps(data).encode("utf-8")  # Encode data as JSON bytestring
  future = publisher.publish(topic_path, data_string)
  future.result()  # Wait for publishing to complete
  print(f"Published message: {data_string.decode('utf-8')}")
...
for _ in range(10):  # Adjust the number of messages to publish
  ...
  if data_type == "clickstream":
    ...
    data = f.inventoryschema_data()
  else:
    data = f.transactionschema_data()
